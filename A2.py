...
class App(tk.Tk):
    ...
    def __init__(self,que,rnk):
        ...
        id_label = tk.Label(self, text="输入你的ID:")
        # 输入ID标签
        self.id_label = id_label
        # 这个用来调用forget
        id_label.pack()

        your_id = tk.Entry(self)
        # 输入玩家id
        ...
        # 积分
        self.quit_button = tk.Button(self, text="退出游戏", command=self.quit_game)
        # 退出按钮
    ...
    def show_ranking(self):
        # 创建排行榜框架
        ranking_frame = tk.Frame(self)
        ranking_frame.pack(pady=10)
        self.ranking_frame = ranking_frame
        
        # 排行榜标题
        rank_title = tk.Label(ranking_frame, text="排行榜", font=("Arial", 14, "bold"))
        rank_title.grid(row=0, column=0, columnspan=3, pady=5)
        
        # 排行榜表头
        headers = ["排名", "玩家ID", "分数","日期"]
        for i, header in enumerate(headers):
            tk.Label(ranking_frame, text=header, font=("Arial", 10, "bold")).grid(row=1, column=i, padx=15)
        
        # 显示排行榜数据
        for i, rank_data in enumerate(self.ranking):
            if rank_data["score"] is not None:
                tk.Label(ranking_frame, text=rank_data["rank"]).grid(row=i+2, column=0)
                tk.Label(ranking_frame, text=rank_data["player_id"]).grid(row=i+2, column=1)
                tk.Label(ranking_frame, text=str(rank_data["score"])).grid(row=i+2, column=2)
                tk.Label(ranking_frame, text=str(rank_data["date"])).grid(row=i+2, column=3)
            else:
                tk.Label(ranking_frame, text=rank_data["rank"]).grid(row=i+2, column=0)
                tk.Label(ranking_frame, text="---").grid(row=i+2, column=1)
                tk.Label(ranking_frame, text="---").grid(row=i+2, column=2)
                tk.Label(ranking_frame, text="---").grid(row=i+2, column=3)
